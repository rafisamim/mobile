...
export default class HomeScreen extends Component {
  ...
  getCategories = async () => {
    await fetch(
      Setting.resourceCategories + this.props.screenProps.i18n.language
    )
      .then((data) => data.json())
      .then((res) => {
        this.setState({
  ...
  getFeaturedResources = async () => {
    await fetch(
      Setting.featuredResources + this.props.screenProps.i18n.language
    )
      .then((data) => data.json())
      .then((res) => {
        this.setState({
  ...
  render() {
    ...
    if (!this.state.categoriesLoading && !this.state.featuredResourcesLoading) {
      ...
      let featuredResources = this.state.featuredResourcesDataSource.map(
        (category, key) => {
          let type = category.subject_id ? "subject_area" : "type";
          let catId = category.subject_id
            ? category.subject_id
            : category.type_id;

          if (
            category.type_id == 2045 ||
            category.type_id == 837 ||
            category.type_id == 3831
          )
            return (
              <TouchableOpacity
                key={key}
          ...
          <ScrollView scrollEventThrottle={16}>
            ...
            <View style={{ flex: 1, marginTop: 10 }}>
              <Text style={styles.title}>
                {t("Featured Resource Collections")}
              </Text>
            </View>
            ...
            <View style={{ flex: 1 }}>
              <Text style={styles.title}>
                {t("Darakht-e Danesh Online Library")}
              </Text>
              <Text style={styles.subTitle}>
                {t("Watch a video to learn more about our work in Afghanistan")}
              </Text>
            </View>
            ...
            >
              ...
              >
                <Image
                  source={require("../assets/images/home/learn_more.png")}
                  style={{
                    flex: 1,
                    height: null,
                    width: null,
